...
def registrarArticulo(folio):
    ...
    query = 'insert into venta_detalle (articulo,precio,cantidad,venta_id) values(\'' + str(descripcion) + '\' ,\'' + str(precio) + '\',\'' + str(cantidad) + '\',\'' + str(folio) + '\')'
    ejecutarComando(query)


    #Una vez que se pide la información Desc, Precio y Cantidad, se almacenan
    #en una lista para posterior almacenarla en uan lista principal (lista anidadad)
...
while(opcion!=4):
    ...
    if opcion == 1:
       ...
       while(subopcion!=2):
         ...
         if(subopcion==1):
            #Si ingresa 1, el programa invocará a la funciión registrarArticulo() para almacenar
            #dicho articulo a la lista subdetalle
            items=[]
    ...
    if opcion ==2:
        ...
        ejecutarConsultaBusqueda("SELECT * FROM venta inner join venta_detalle on venta.venta_id=venta_detalle.venta_id where venta.venta_id= "+str(folio))
